...
public class CmisNavigationService {
	...
	public static class Impl {
		...
		private static ObjectInFolderList getChildrenIntern(String repositoryId, String folderId, String filter,
			...
			MNavigationServiceDAO navigationMorphiaDAO = DatabaseManager.getInstance(repositoryId)
			...
			if (data.getName().equalsIgnoreCase("@ROOT@")) {
				...
				List<MAccessControlListImpl> mAcl = getParentAcl(repositoryId, data.getInternalPath(), data.getAcl());
		...
		private static List<ObjectInFolderContainer> getDescendantsIntern(String repositoryId, String folderId,
			...
			MDocumentObjectDAO docMorphiaDAO = DatabaseManager.getInstance(repositoryId).getObjectService(repositoryId,
		...
		private static List<ObjectInFolderContainer> getDescendants(String repositoryId, List<MBaseObject> children,
				...
				IUserObject userObject, MDocumentObjectDAO docMorphia) {
			List<ObjectInFolderContainer> childrenOfFolderId = new ArrayList<ObjectInFolderContainer>();
		...
		private static ObjectInFolderList getChildernList(String repositoryId, List<MBaseObject> children,
				String folderId, Boolean includePathSegments, String filter, Boolean includeAllowableActions,
				ObjectInfoHandler objectInfos, String renditionFilter, IncludeRelationships includeRelationships,
				IUserObject userObject, MDocumentObjectDAO docMorphiaDAO) {
			List<ObjectInFolderData> folderList = new ArrayList<ObjectInFolderData>();
			ObjectInFolderListImpl result = new ObjectInFolderListImpl();
			for (MBaseObject child : children) {
		...
		private static ObjectList getCheckedOutIntern(String repositoryId, String folderId, String filter,
			...
			List<MDocumentObject> document = new ArrayList<>();
			...
			for (MDocumentObject checkedOutDocs : document) {
				Set<String> filterCollection = splitFilter(filter);
		...
		public static List<MAccessControlListImpl> getParentAcl(String repositoryId, String dataPath, MAccessControlListImpl dataAcl) {
			List<MAccessControlListImpl> acl = null;
